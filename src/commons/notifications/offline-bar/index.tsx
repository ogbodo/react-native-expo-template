...
export default function OfflineBar({ text }: { text?: string }) {
  const defaultText = 'You are not connected to Internet';
  ...
  const setNetworkStatus = (state: NetInfoState) => {
    ...
    switch (true) {
      case isConnected && !isInternetReachable:
        setNetwork({ ...network, text: 'Internet is not reachable' });
        break;
